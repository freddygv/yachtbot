...
func (api *Client) DeleteMessage(channelID, messageTimestamp string) (string, string, error) {
	respChannel, respTimestamp, _, err := api.SendMessageContext(context.Background(), channelID, MsgOptionDelete(messageTimestamp))
...
func (api *Client) DeleteMessageContext(ctx context.Context, channelID, messageTimestamp string) (string, string, error) {
	respChannel, respTimestamp, _, err := api.SendMessageContext(ctx, channelID, MsgOptionDelete(messageTimestamp))
	return respChannel, respTimestamp, err
}
...
func (api *Client) PostMessage(channelID, text string, params PostMessageParameters) (string, string, error) {
...
func (api *Client) PostMessageContext(ctx context.Context, channelID, text string, params PostMessageParameters) (string, string, error) {
	respChannel, respTimestamp, _, err := api.SendMessageContext(
		ctx,
		channelID,
		MsgOptionText(text, params.EscapeText),
...
func (api *Client) PostEphemeralContext(ctx context.Context, channelID, userID string, options ...MsgOption) (string, error) {
	path, values, err := ApplyMsgOptions(api.Config.token, channelID, options...)
	if err != nil {
		return "", err
	}
...
func (api *Client) SendMessage(channelID string, options ...MsgOption) (string, string, string, error) {
	return api.SendMessageContext(context.Background(), channelID, options...)
}

// SendMessageContext more flexible method for configuring messages with a custom context.
func (api *Client) SendMessageContext(ctx context.Context, channelID string, options ...MsgOption) (string, string, string, error) {
	path, values, err := ApplyMsgOptions(api.Config.token, channelID, options...)
	if err != nil {
		return "", "", "", err
	}
...
func ApplyMsgOptions(token, channelID string, options ...MsgOption) (string, url.Values, error) {
	config := sendConfig{
...
// MsgOptionPostEphemeralParameters maintain backwards compatibility.
